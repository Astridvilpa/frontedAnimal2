...
export default function Appointments({ isAdmin }) {
  ...
  const [appointmentForm, setAppointmentForm] = useState({ appointment_date: "", service_id: "", veterinario_id: "" });
  ...
  useEffect(() => {
    ...
    const fetchVeterinarios = async () => {
      try {
        const response = await getAllVeterinarios(token);
        if (response.success) {
          setVeterinarios(response.data);
        } else {
          setError("Error al obtener veterinarios.");
        }
      } catch (err) {
        setError("Error al obtener veterinarios.");
      }
  ...
  const handleCreateAppointment = async () => {
    try {
      ...
      if (response.success) {
        ...
        setAppointmentForm({ appointment_date: "", service_id: "", veterinario_id: "" });
      } else {
          ...
          <Col key={appointment.id} md={4} className="mb-4">
            <Card>
              <Card.Body>
                ...
                <Card.Text>
                  Veterinario: {veterinarios.find(veterinario => veterinario.id === appointment.veterinario_id)?.name || "No asignado"}
                </Card.Text>
              </Card.Body>
            </Card>
          </Col>
        ...
        <Form.Group controlId="formVeterinarioId">
          ...
          >
            ...
            {veterinarios.map(veterinario => (
              <option key={veterniario.id} value={veterinario.id}>{veterinario.name}</option>
            ))}
          </Form.Control>
        </Form.Group>
